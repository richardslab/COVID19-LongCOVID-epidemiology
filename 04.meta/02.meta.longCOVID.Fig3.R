...
write.xlsx(OUT, file = "/home/richards/tomoko.nakanishi/my_project/repo/COVID19-LongCOVID-epidemiology/results/longCOVID_individual.xlsx")
...
write.xlsx(data, file = "/home/richards/tomoko.nakanishi/my_project/repo/COVID19-LongCOVID-epidemiology/results/meta.longCOVID.xlsx")
data1 <- data %>% filter(meta == "fixed" & sx == "longCOVID")

data1$covariates <- rep(c("Hospitalization", "<60", "70-80", ">80", "Female", "BMI≥30", 
...
data1$covariates <- factor(data1$covariates, levels = rev(c("Hospitalization", "<60", "60-70 (ref)",  "70-80",">80",
                                                        "Female", "BMI≥30", 
...
data1 <- data1 %>% mutate(group = case_when(covariates %in% c("Hospitalization") ~ "1",
                                            covariates %in% c("<60", "70-80", ">80", "60-70 (ref)") ~ "2",
...
png("/home/richards/tomoko.nakanishi/my_project/repo/COVID19-LongCOVID-epidemiology/results/Fig2.longCOVID.png", width = 700, height = 600)
data1 %>% 
  ...
  scale_y_continuous(breaks=c(0.3, 0.5, 1.0, 1.5, 2), limit=c(0.27, 2.2)) + scale_alpha_discrete(range = c(0.5, 1)) + 
