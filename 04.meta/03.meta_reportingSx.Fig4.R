...
OUT$covariates[OUT$covariates == "hospital"] <- "hospitalization"
table(OUT[,c("covariates", "group")])
library(meta)

OUT <- OUT %>% mutate(Sx = case_when(Sx == "Sx_any" ~ "sx_any",
                                     TRUE ~ Sx))
...
for(i in c("case", "control")){
  l <- "sx_any"
    ...
    for(j in seq(1, length(cov))){
      tmp <- OUT %>% filter(casecontrol == i & covariates == cov[j] & Sx == l) 
      m1 <- metagen(Estimate,
                    Std..Error,
                    data=tmp,
                    studlab=paste(group),
...
data1$covariates <- rep(c( "70-79","<60", "≥80", "Female", "Hospitalization","BMI≥30", 
                          "Current smoker", "Past smoker", "Asthma", "COPD", "Hypertension", "Diabetes", "Autoimmune Disease",
                          "Cancer", "Dementia"), 2)

tmp <- data1[1:4,]
tmp$covariates <- rep(c("60-69 (ref)", "Never smoker (ref)"), 2)
tmp$beta <- 0
tmp$se <- 0
tmp$pval <- 1
tmp$meta <- "fixed"
...
data1$covariates <- factor(data1$covariates, levels = c("Hospitalization","<60", "60-69 (ref)",  "70-79","≥80",
                                                        "Female", "BMI≥30", "Never smoker (ref)",
                                                        "Current smoker", "Past smoker", "Asthma", "COPD", "Autoimmune Disease","Hypertension", "Diabetes", 
                                                        "Cancer", "Dementia"))
...
data1 %>% filter(covariates == "Autoimmune Disease")
#breaks=c(0.1, 0.3, 0.5, 1, 2, 6, 10)
png("/home/richards/tomoko.nakanishi/my_project/repo/COVID19-LongCOVID-epidemiology/results/Fig.4.anySx_casecontrol_fixed.png", width = 900, height = 800)
data1 %>% 
  ...
  scale_color_manual(values=c("#FF8000","#00994C"), label = c("non-COVID-19","COVID-19")) + 
