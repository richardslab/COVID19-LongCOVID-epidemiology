...
data$group <- i
data$sx <- sx[l]
for(j in c(1:8, 10, 12:length(cov))){
  tmp <- OUT %>% filter(casecontrol == i & covariates == cov[j] & Sx == sx[l]) 
  m1 <- metagen(Estimate,
                    Std..Error,
                    data=tmp,
...
for(j in c(9, 11)){
  tmp <- OUT %>% filter(casecontrol == i & covariates == cov[j] & Sx == sx[l]) 
  data$covariates[(2*j-1):(2*j)] <- cov[j]
  data[(2*j-1),2:4] <- tmp[1, c("Estimate", "Std..Error", "Pr...z..")]
  data[(2*j),2:4] <- tmp[1, c("Estimate", "Std..Error", "Pr...z..")]
  data[(2*j-1):(2*j),5] <- c("fixed", "random")
}
write.table(data, file="meta_summary.logreg.tsv", sep="\t", quote=F, col.names = F, row.names = F, append=T)
...
data1$covariates <- rep(c("50-60", "70-80", ">80", "Female", "BMI≥30", 
                          "Current smoker", "Past smoker", "Asthma", "COPD", "Hypertension", "Diabetes", "Autoimmune",
                          "Cancer", "Dementia", "<50"), 35)
...
data1$covariates <- factor(data1$covariates, levels = c("<50", "50-60", "60-70 (ref)",  "70-80",">80",
...
png("anySx_casecontrol_fixed.png", width = 700, height = 800)
data1 %>% filter(sx == "Sx_any") %>% 
  ...
  scale_fill_manual(values=c("#FF8000","#00994C")) + scale_y_log10() + scale_alpha_discrete(range = c(0.5, 1)) + 
