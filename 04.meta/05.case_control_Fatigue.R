...
OUT <- OUT %>% mutate(cohort = case_when(grepl("UKB", group) ~ "UKB",
                      ...
                      phenotype = case_when(grepl("fatigue", group) ~ "Fatigue",
                                            grepl("SOB", group) ~ "SOB",
                                            grepl("cough", group) ~ "Cough",
                                            grepl("Any", group) ~ "Any"),
...
data$group <- "cases"
data$phenotype <- "Fatigue"
library(meta)
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "cases" & covariates == "hospital")
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "cases" & covariates %in% c("age_range50-60"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "cases" & covariates %in% c("age_range70-80"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "cases" & covariates %in% c("age_range>80"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "cases" & covariates %in% c("sexF"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "cases" & covariates %in% c("obesity"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "cases"  & covariates %in% c("smokingcurrent"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "cases" & covariates %in% c("smokingpast"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "cases" & covariates %in% c("com_asthma"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "cases" & covariates %in% c("com_copd"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "cases" & covariates %in% c("com_hypertension"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "cases" & covariates %in% c("com_diabetes"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "cases" & covariates %in% c("com_autoimmune"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "cases" & covariates %in% c("com_cancer"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "cases" & covariates %in% c("com_dementia"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
write.table(data, file="/home/richards/tomoko.nakanishi/scratch/09.COVID19/08.LongCOVID_epi/Fatigue.cases.tsv", sep="\t", quote=F, col.names = T, row.names = F)
...
data$phenotype <- "Fatigue"
library(meta)
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "controls" & covariates == "hospital")
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "controls" & covariates %in% c("age_range50-60"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "controls" & covariates %in% c("age_range70-80"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "controls" & covariates %in% c("age_range>80"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "controls" & covariates %in% c("sexF"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "controls" & covariates %in% c("obesity"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "controls"  & covariates %in% c("smokingcurrent"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "controls" & covariates %in% c("smokingpast"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "controls" & covariates %in% c("com_asthma"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "controls" & covariates %in% c("com_copd"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "controls" & covariates %in% c("com_hypertension"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "controls" & covariates %in% c("com_diabetes"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "controls" & covariates %in% c("com_autoimmune"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "controls" & covariates %in% c("com_cancer"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
tmp <- OUT %>% filter(phenotype == "Fatigue" & casecontrol == "controls" & covariates %in% c("com_dementia"))
m1 <- metagen(Estimate,
              Std..Error,
              data=tmp,
              studlab=paste(cohort),
...
write.table(data, file="/home/richards/tomoko.nakanishi/scratch/09.COVID19/08.LongCOVID_epi/Fatigue.controls.tsv", sep="\t", quote=F, col.names = T, row.names = F)


out1 <- fread("/home/richards/tomoko.nakanishi/scratch/09.COVID19/08.LongCOVID_epi/Fatigue.cases.tsv")
out2 <- fread("/home/richards/tomoko.nakanishi/scratch/09.COVID19/08.LongCOVID_epi/Fatigue.controls.tsv")
...
png("case_control_Fatigue.png", width = 700, height = 700)
data1 %>%
  ...
  scale_shape_identity() + ggtitle("Fatigue") + 
